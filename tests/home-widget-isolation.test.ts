 ...
 * getHomeDashboard runs each widget through settleWidget (fallback on failure);
 * Promise.all over settleWidget therefore never rejects → /api/home never 500s
 * from a widget. These cover the fault-isolation core + safe logging. The DB-backed
 * end-to-end (route returns 200 with partial data) is an integration skip.
 */
...
test("G: settleWidget does NOT swallow auth errors (auth runs OUTSIDE it)", async () => {
  // getHomeDashboard receives an already-authenticated user; requireUser is in the
  // route, before getHomeDashboard. settleWidget only wraps the 6 data calls, so an
  // auth failure never reaches it. Model that: a throw in the "auth" step (a plain
  // call, not wrapped) propagates.
