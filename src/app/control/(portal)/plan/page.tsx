...
import { CheckCircle2, ChevronDown, Circle, Plus, Power, Trash2, X } from "lucide-react";
import { managerApi } from "@/lib/api/club-plan-client";
import { ApiError } from "@/lib/api/client";
import type { ClubPlanDTO, ClubPlanEmployeeDTO, ClubTaskTarget } from "@/lib/api/club-plan-types";
import { Field, TextArea, TextInput, fieldCls } from "@/components/admin/ui";

const POSITION_LABEL: Record<string, string> = {
  CLIENT_MANAGER: "Менеджеры по работе с клиентами",
  NIGHT_MANAGER: "Ночные менеджеры",
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-end gap-3">
          ...
          <button
            ...
            disabled={!plan?.clubId}
            className="flex h-11 items-center gap-1.5 rounded-2xl bg-brand px-5 text-sm font-semibold text-brand-foreground disabled:opacity-50"
          >
        ...
        <>
          ...
          <div className="mt-6 space-y-2">
            {plan.employees.map((e) => (
              <EmployeeRow key={e.userId} emp={e} onChanged={load} />
            ))}
            ...
            <ul className="space-y-1.5">
              {emp.tasks.map((t) => (
                <li key={t.id} className="flex items-center gap-2 text-sm">
                  ...
                  {t.required && <span className="shrink-0 rounded-full bg-brand/12 px-2 py-0.5 text-[11px] font-semibold text-brand">обяз.</span>}
                  {t.isManager && <span className="shrink-0 rounded-full bg-muted px-2 py-0.5 text-[11px] text-muted-foreground">руководитель</span>}
                  {t.canDelete && (
                    <button
                      onClick={async () => { if (confirm("Удалить задачу?")) { await managerApi.deleteTask(t.id); await onChanged(); } }}
                      className="shrink-0 rounded-lg p-1 text-red-500 hover:bg-red-500/10"
                    >
...
function StandardPlan({ plan, onChanged }: { plan: ClubPlanDTO; onChanged: () => Promise<void> }) {
  ...
  const add = async () => {
    ...
    setBusy(true);
    try { await managerApi.createTemplate({ title: title.trim() }); setTitle(""); await onChanged(); }
    finally { setBusy(false); }
  };
  return (
    <section className="mt-10 border-t border-border pt-6">
      <h2 className="font-bold">Стандартный план (ежедневно)</h2>
      <p className="mt-1 text-sm text-muted-foreground">Повторяющиеся задачи материализуются в план сотрудников каждый день.</p>
      ...
      <div className="mt-4 space-y-2">
        {plan.templates.map((t) => (
          <div key={t.id} className="flex items-center gap-3 rounded-2xl border border-border bg-card px-4 py-2.5">
            <div className="min-w-0 flex-1">
              <p className={`truncate font-medium ${!t.isActive ? "text-muted-foreground line-through" : ""}`}>{t.title}</p>
              <p className="truncate text-xs text-muted-foreground">
                {t.targetPosition ? POSITION_LABEL[t.targetPosition] : "Все менеджеры"}{t.required ? " · обязательная" : ""}
              </p>
            </div>
            <button
              onClick={async () => { await managerApi.updateTemplate(t.id, { isActive: !t.isActive }); await onChanged(); }}
              className={`flex h-8 items-center gap-1 rounded-xl px-2.5 text-xs font-semibold ${t.isActive ? "bg-success-soft text-success" : "bg-muted text-muted-foreground"}`}
            >
      ...
      <div className="mt-4 flex max-w-md items-center gap-2">
        <input className={fieldCls} placeholder="Новая стандартная задача" value={title} onChange={(e) => setTitle(e.target.value)} onKeyDown={(e) => e.key === "Enter" && add()} />
        <button onClick={add} disabled={busy || title.trim().length < 2} className="flex h-11 shrink-0 items-center gap-1 rounded-2xl bg-brand px-4 text-sm font-semibold text-brand-foreground disabled:opacity-50">
...
function CreateTaskModal({
  ...
  const create = async () => {
    ...
    setError(null);
    try {
      await managerApi.createTask({ title: title.trim(), description: description || null, date, required, target });
      await onCreated();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl border border-border bg-card p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 space-y-3">
          <Field label="Название"><TextInput value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Например: Обзвонить клиентов с заканчивающейся картой" /></Field>
